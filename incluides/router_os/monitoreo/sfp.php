...
$sql_puerca = mysqli_query($conexion, "UPDATE `puerto` SET `tx_sfp`='0', `rx_sfp`='0'");
var_dump($sql_puerca);
    ...
    while ($data = mysqli_fetch_assoc($queryp)) {
        ...
        if ($API->connect($ip_mk, $user, $pass)) {
            ...
            foreach ($ARRAY as $y) {
                ...
                //echo($com);
                //echo (' ');
                $nm = $p1['sfp-wavelength'];
                //echo($com);
                //echo (' ');
                $cadena = utf8_encode($name);

                $sql_temp = mysqli_query($conexion, "UPDATE `puerto` SET `tx_sfp`='$tx', `rx_sfp`='$rx',`nm`='$nm' WHERE ip_mk = '$ip_mk' && `puerto` = '$cadena' ");
                var_dump($sql_temp);
