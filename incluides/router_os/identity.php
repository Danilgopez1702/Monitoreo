...
//Extraccion de datos del array
            ...
            //echo($com);
            //echo (' ');
            $cadena = utf8_encode($name);

            $quer = mysqli_query($conexion, "SELECT * FROM principal WHERE nombre = '$cadena'");
            $pa = mysqli_num_rows($quer);

            if($pa < 0){
                  $sql_temp = mysqli_query($conexion, "UPDATE `principal` SET `nombre`='$cadena' WHERE ip_mk = '$ip_mk'");
                  var_dump($sql_temp);

                  $sql_t = mysqli_query($conexion, "UPDATE `puerto` SET `nombre_mk`='$cadena' WHERE ip_mk = '$ip_mk'");
                  var_dump($sql_t);
            }else{
